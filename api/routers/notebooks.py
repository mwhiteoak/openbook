...
def _assert_notebook_owner(notebook: Notebook, current_user: dict) -> None:
    """Raise 404 if the notebook doesn't belong to the current user.

    Using 404 (not 403) avoids leaking the existence of notebooks owned by
    other users.
    """
    if not notebook.user_id:
        raise HTTPException(status_code=404, detail="Notebook not found")
    try:
        nb_owner = str(ensure_record_id(notebook.user_id))
        requester = str(ensure_record_id(current_user["id"]))
    except Exception:
        raise HTTPException(status_code=404, detail="Notebook not found")
    if nb_owner != requester:
        raise HTTPException(status_code=404, detail="Notebook not found")
...
async def get_notebooks(
    ...
    try:
        ...
        user_rid = ensure_record_id(current_user["id"])
        query = f"""
            SELECT *,
        ...
        result = await repo_query(query, {"user_id": user_rid})
...
async def get_notebook_delete_preview(
    ...
    try:
        ...
        if not notebook:
            raise HTTPException(status_code=404, detail="Notebook not found")
        _assert_notebook_owner(notebook, current_user)
...
async def get_notebook(
    ...
    try:
        # Fetch the notebook first to verify ownership
        notebook = await Notebook.get(notebook_id)
        if not notebook:
            raise HTTPException(status_code=404, detail="Notebook not found")
        _assert_notebook_owner(notebook, current_user)
...
async def add_source_to_notebook(
    ...
    try:
        # Check if notebook exists and is owned by current user
        notebook = await Notebook.get(notebook_id)
        if not notebook:
            raise HTTPException(status_code=404, detail="Notebook not found")
        _assert_notebook_owner(notebook, current_user)
