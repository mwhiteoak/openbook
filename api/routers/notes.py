...
def _assert_note_owner(note: Note, current_user: dict) -> None:
    """Raise 404 if the note doesn't belong to the current user."""
    if not note.user_id:
        raise HTTPException(status_code=404, detail="Note not found")
    try:
        note_owner = str(ensure_record_id(note.user_id))
        requester = str(ensure_record_id(current_user["id"]))
    except Exception:
        raise HTTPException(status_code=404, detail="Note not found")
    if note_owner != requester:
        raise HTTPException(status_code=404, detail="Note not found")
...
async def get_notes(
    ...
    """Get all notes for the current user with optional notebook filtering."""
    try:
        if notebook_id:
            ...
            raw = await repo_query(
                "SELECT * omit content, embedding FROM note WHERE user_id = $uid ORDER BY updated DESC",
                {"uid": ensure_record_id(current_user["id"])},
            )
...
async def get_note(
    ...
    try:
        ...
        if not note:
            raise HTTPException(status_code=404, detail="Note not found")
        _assert_note_owner(note, current_user)
